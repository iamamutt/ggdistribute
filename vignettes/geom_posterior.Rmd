...
The `PositionSpread` class spreads out overlapping densities within the range of their y axis value. For instance, there are two different groups for `cond` A-D, but only one group in E. Distributions within each `cond` are resized and spread over the group's y interval. Padding is turned off below to see where distributions begin and end.
