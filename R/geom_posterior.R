...
geom_posterior <- function(mapping = NULL, data = NULL, stat = "DensityCI",
                           ...
                           brighten = TRUE, mirror = FALSE,
                           interp_thresh = NULL, na.rm = FALSE,
                           show.legend = NA, inherit.aes = TRUE) {
  layer(
    geom = GeomPosterior, stat = stat, data = data,
...
GeomPosterior <- ggproto(
  ...
  Geom,
  default_aes = aes(
    weight = 1, colour = "#585872", fill = "#8989B2",
    size = 0.5, alpha = NA, linetype = "solid", vjust = 1),
  required_aes = c("x", "y"),
...
setup_post_line_params <- function(data, midline_color, draw_ci, draw_sd, ...) {
  ...
  for (j in seq_len(n)) {
    line_list[[j]] <- list(
      ci_column = line_pos[keep[j]],
      ci_color = line_colors[keep[j]],
...
compute_post_seg_data <- function(data, lower_cut = NULL, upper_cut = NULL,
  ...
  if (!any(within_range)) {
    warning(sprintf(
      ...
      lower_cut, upper_cut), call. = FALSE)
...
compute_post_line_data <- function(data, ci_column, ci_color = "#000000",
                                   ci_lwd_adj = 1) {
  ...
  grob_data <- data.table(
    x = c(vline, vline),
    y = interp_vert_line(dt$x, dt$ylower, dt$yupper, vline),
    group = c(vline, vline)
  )
