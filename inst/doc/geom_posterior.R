...
theme_update(
  ...
  panel.border = element_rect(fill = NA, colour = gray(0.84), size = 0.67),
...
## ------------------------------------------------------------------------
ex_plt <- ggplot(sre_data(5000), aes(y = effect)) +
  ...
  geom_posterior(
    ...
    brighten = c(1.6, 1, 1.2), interp_thresh = .001,
...
if (requireNamespace("mejr", quietly = TRUE) && rebuild_showcase_plot) {
  mejr::save_plot(ex_plt,
    file = file.path("media", "example_fig"),
    format = "png", width = 5, height = 3, font = font, res = 600)
}
